...
if($_REQUEST['submit']){
    ...
    if (!$link) {
        ...
        if ($result == false) {
            ...
            echo "<pre>";
            echo "Спасибо за ваш заказ! Ему присвоен номер $order_id" . PHP_EOL;
            echo "Получатель: " . $_REQUEST['user_last_name'] . ' ' . $_REQUEST['user_name'] . ' ' . $_REQUEST['user_second_name'] . PHP_EOL;
            echo "Адрес доставки: г." . $_REQUEST['user_address_city'] . ', улица ' . $_REQUEST['user_address_street'] . ', дом ' . $_REQUEST['user_address_house'] . ', квартира ' . $_REQUEST['user_address_flat'];
            echo "</pre>";
        }
    }
}
